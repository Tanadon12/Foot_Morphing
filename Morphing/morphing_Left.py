...
def reset_object_scale(obj_name):
    ...
    current_dimensions = get_object_dimensions(obj_name)
...
def main_sole(obj_name,model_dict):
    ...
    for key in vg_point_D:
        value = value_dict[key]
        target_x = value  # Real-life length in CM
        target_co = mathutils.Vector((target_x * 10, -50, 5))  # Convert to Blender units (1 CM = 10 M)
        group_name = f"Point_{key}"  # Name of the vertex group
        create_vertex_group(obj, group_name, target_co, num_vertices)
    ...
    for key in vg_point_E:
        value = value_dict[key]
        target_x = value  # Real-life length in CM
        target_co = mathutils.Vector((target_x * 10, 40, 5))  # Convert to Blender units (1 CM = 10 M)
        group_name = f"Point_{key}"  # Name of the vertex group
        create_vertex_group(obj, group_name, target_co, num_vertices)
    ...
    for key in vg_point_E:
        ...
        create_vertex_group(obj, group_name, target_co, num_vertices)
    # right most and left most actually is it a position of d and e but this is use for to calculate a scale object on Y axis 
...
def main():
    ...
    object_name = 'AI-001-SUTHAWAN_BUCHATHIP_Left'
    obj = bpy.data.objects.get(object_name)
